 ...
 *
 * Each VisualEdge becomes a straight line from its source node's center to its
 * target node's center. The edge's `state` decides both color and line width:
 * neutral edges are thin and gray, while active/path/highlighted edges are
 * thicker and brightly colored so the algorithm's current focus pops.
 *
 * Directed edges end in a small filled triangle (arrowhead) oriented along
 * the line's direction. Lines run to the very center of the target node;
...
/** Color / width pairs resolved from an edge's state. */
const EDGE_STYLES: Partial<Record<EntityState, { color: string; width: number }>> = {
    idle: { color: "#9CA3AF", width: 2 },
    active: { color: "#3B82F6", width: 3 },
    path: { color: "#06B6D4", width: 3 },
    highlight: { color: "#F472B6", width: 3 },
};
...
export function renderEdges(ctx: CanvasRenderingContext2D, frame: VisualFrame): void {
    ...
    for (const edge of frame.edges) {
        ...
        if (edge.directed) {
            drawArrowhead(ctx, source.x, source.y, target.x, target.y, style.color);
        }
 ...
 * @param x1 Source center x.
 * @param y1 Source center y.
 * @param x2 Target center x (arrowhead point).
 * @param y2 Target center y (arrowhead point).
