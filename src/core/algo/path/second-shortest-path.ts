...
function* run(input: unknown): Generator<VisualFrame, void, unknown> {
    ...
    const dij = (ban: string | null): { path: string[]; cost: number } | null => {
    ...
    const first = dij(null);
    if (!first) {
        yield snap(`${target} unreachable from ${start}.`, 0, {});
        return;
    }
    for (const v of first.path) setN(v, "path");
    yield snap(
        `Shortest P1 = ${first.path.join("→")} (cost ${first.cost}). Deviate once per edge.`,
        0,
        { best: first.cost },
    );
    step += 1;
    let runner: { path: string[]; cost: number } | null = null;
    for (let i = 0; i + 1 < first.path.length; i += 1) {
        const a = first.path[i] as string;
        const b = first.path[i + 1] as string;
        const alt = dij(`${a}|${b}`);
        clr();
        setE(a, b, "swapped");
        if (alt && alt.path.join(",") !== first.path.join(",")) {
            for (const v of alt.path) setN(v, "comparing");
            if (!runner || alt.cost < runner.cost) runner = alt;
            yield snap(`Spur avoiding ${a}→${b}: ${alt.path.join("→")} costs ${alt.cost}.`, 1, {
                candidate: alt.cost,
            });
        } else {
            yield snap(`Spur avoiding ${a}→${b}: no valid deviation.`, 1, { candidate: -1 });
        }
        step += 1;
    }
    yield snap(
        runner
            ? `Second shortest = ${runner.path.join("→")} (cost ${runner.cost}).`
            : "No second path exists.",
        2,
        { second: runner ? runner.cost : -1 },
    );
}
