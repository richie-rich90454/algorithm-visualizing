/**
 * avl-tree.ts – AVL Tree (self-balancing BST)
 *
 * ---------------------------------------------------------------------------
 * What it does
 * ---------------------------------------------------------------------------
 ...
 * ---------------------------------------------------------------------------
 *   - The node whose balance factor becomes ±2 is YELLOW (comparing).
 ...
 *
 * @param input `{ inserts }` – values inserted in order.
 */
function* run(input: unknown): Generator<VisualFrame, void, unknown> {
    ...
    for (const value of inserts) {
        ...
        yield buildFrame(`Inserted ${value} – rotations keep every node balanced.`);
        step += 1;
    }

    yield buildFrame(`AVL tree complete with ${inserts.length} values – height is O(log n).`);
}
