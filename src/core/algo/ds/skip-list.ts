 ...
 * 8, … positions. Searching starts at the topmost level and drops down a level
 ...
 *
 * @param input `{ values, search }` – values to insert and a value to search.
 */
function* run(input: unknown): Generator<VisualFrame, void, unknown> {
    ...
    for (let levelIndex = 0; levelIndex < levels.length; levelIndex += 1) {
        ...
        yield {
            ...
            entities: current,
            edges: edges.map((e) => ({ ...e })),
            description: `Searching level ${levelIndex} for ${search}${found ? " – FOUND!" : " – overshot, dropping down"}.`,
            codeLineNumber: 2,
            layout: "graph",
            meta: { search },
        };
    ...
    yield {
        ...
        entities: entities.map((e) => ({ ...e })),
        edges: edges.map((e) => ({ ...e })),
        description: `Skip list search visits only a few nodes – O(log n) expected.`,
        codeLineNumber: 3,
        layout: "graph",
        meta: { search },
    };
