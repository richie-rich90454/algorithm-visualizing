/**
 * dynamic-segment-tree.ts – Dynamic Segment Tree
 *
 * ---------------------------------------------------------------------------
 * What it does
 * ---------------------------------------------------------------------------
 ...
 * Complexity
 * ---------------------------------------------------------------------------
 *   Update / query: O(log MAX) – one new node per visited path segment
 *   Space:          O(ops · log MAX)
 *
 * ---------------------------------------------------------------------------
 ...
 *
 * @param input `{ inserts }` – points to insert (sparse).
 */
function* run(input: unknown): Generator<VisualFrame, void, unknown> {
    ...
    yield buildFrame("Dynamic segment tree over [0, 15] – only the root exists.");
    step += 1;

    for (const pos of inserts) {
        insert(root, 0, MAX - 1, pos);
        yield buildFrame(`Inserted ${pos} – created only the O(log MAX) nodes on its path.`);
        step += 1;
    }

    yield buildFrame(
        `Dynamic tree complete – ${nextId} node(s) created lazily, far fewer than the full range.`,
    );
}
