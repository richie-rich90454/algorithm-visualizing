/**
 * xor-linked-list.ts ‚Ä?XOR Linked List (memory-efficient doubly linked list)
 *
 * ---------------------------------------------------------------------------
 * What it does
 * ---------------------------------------------------------------------------
...
function* run(input: unknown): Generator<VisualFrame, void, unknown> {
    ...
    yield {
        ...
        edges: [],
        description: "XOR linked list ‚Ä?each node stores prev ‚ä?next.",
        codeLineNumber: 0,
        layout: "grid",
        meta: { size: n },
    };
    ...
    while (current >= 0 && current < n) {
        yield {
            ...
            edges: [],
            description: `At node ${current} (value ${values[current]}) ‚Ä?next = ${prev} ‚ä?${xors[current]} = ${prev ^ (xors[current] ?? 0)}.`,
            codeLineNumber: 2,
            layout: "grid",
            meta: { size: n, current },
        };
