/**
 * incidence-matrix.ts 鈥?Incidence Matrix
 *
 * ---------------------------------------------------------------------------
 * What it does
 * ---------------------------------------------------------------------------
 * An incidence matrix has one row per vertex and one column per edge; entry
 * [v][e] is 1 if edge e touches vertex v. For directed graphs it uses 鈭? for
 * the tail and +1 for the head. This representation makes "which edges touch
 * this vertex?" a single column scan, but it uses O(V路E) space.
 *
 * ---------------------------------------------------------------------------
 * Complexity
 * ---------------------------------------------------------------------------
 ...
 * Properties
 * ---------------------------------------------------------------------------
 *   - The vertex脳edge grid is the entire idea.
 */
 ...
 *
 * @param input `{ vertices, edges, inspect }` 鈥?vertex labels, edge triples,
...
function* run(input: unknown): Generator<VisualFrame, void, unknown> {
    ...
    yield {
        ...
        entities: makeCells(),
        edges: [],
        description: `Incidence matrix (${vertices.length} vertices 脳 ${edges.length} edges) 鈥?inspecting edge ${inspect}.`,
        codeLineNumber: 0,
        layout: "grid",
    ...
    yield {
        ...
        edges: [],
        description: `Edge ${inspect} (${edges[inspect]?.join("鈥?)}) touches the 卤1 rows 鈥?readable in one column scan.`,
        codeLineNumber: 2,
        layout: "grid",
        meta: { vertices: vertices.length, edges: edges.length },
    };
    ...
    yield {
        ...
        entities: makeCells(),
        edges: [],
        description: `The matrix costs O(V路E) cells 鈥?dense, but each edge column is self-contained.`,
        codeLineNumber: 3,
        layout: "grid",
        meta: { vertices: vertices.length, edges: edges.length },
    };
...
const module: AlgorithmModule = {
    ...
    complexity: { time: "O(V) edge check", space: "O(V路E)" },
    defaultInput: {
        vertices: ["A", "B", "C", "D"],
        edges: [
            ["A", "B"],
