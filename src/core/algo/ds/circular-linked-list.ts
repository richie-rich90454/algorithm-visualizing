/**
 * circular-linked-list.ts – Circular Linked List
 *
 * ---------------------------------------------------------------------------
 * What it does
 * ---------------------------------------------------------------------------
 * A circular linked list connects the last node back to the first, forming a
 * ring. There is no "head" or "tail" in the ordinary sense – any node can be
...
function* run(input: unknown): Generator<VisualFrame, void, unknown> {
    ...
    yield {
        ...
        edges: edges.map((e) => ({ ...e })),
        description: `Circular linked list: [${values.join(" → ")}] → back to ${values[0]}.`,
        codeLineNumber: 0,
        layout: "graph",
    ...
    for (let i = 0; i < values.length; i += 1) {
        ...
        yield {
            ...
            entities: current,
            edges: edges.map((e) => ({ ...e })),
            description: `Cycling – visiting ${values[i]}.`,
            codeLineNumber: 2,
            layout: "graph",
            meta: { size: values.length },
        };
