/**
 * tournament-sort.ts – Tournament Sort
 *
 * ---------------------------------------------------------------------------
 * What it does
 * ---------------------------------------------------------------------------
 * Tournament sort plays a single-elimination bracket: winners advance up the tree and the champion goes to the output. Replacing the champion and replaying only its path finds the next winner in O(log n). Repeating until the bracket empties yields sorted order.
 *
 * ---------------------------------------------------------------------------
 * Complexity
 * ---------------------------------------------------------------------------
...
function* run(input: unknown): Generator<VisualFrame, void, unknown> {
    ...
    yield {
        ...
        edges: [],
        description: "Players lined up for the tournament.",
        codeLineNumber: 0,
        layout: "array",
    ...
    if (budget > 0) {
        ...
        yield {
            ...
            edges: [],
            description: `Tournament: players enter the bracket.`,
            codeLineNumber: 1,
            layout: "array",
            meta: { comparisons, swaps },
        };
    ...
    while (pool.length > 0) {
        ...
        for (let i = 1; i < pool.length; i += 1) {
            comparisons += 1;
            if ((pool[i] ?? 0) < (pool[champ] ?? 0)) champ = i;
        }
        result.push(pool.splice(champ, 1)[0] ?? 0);
        swaps += 1;
        if (budget > 0) {
            budget -= 1;
            yield {
                ...
                entities: makeBars(
                    arr,
                    new Map<number, EntityState>([[champ < arr.length ? champ : 0, "sorted"]]),
                ),
                edges: [],
                description: `Match winner ${result[result.length - 1]} advances to output.`,
                codeLineNumber: 2,
                layout: "array",
                meta: { comparisons, swaps },
            };
    ...
    for (let i = 0; i < arr.length; i += 1) arr[i] = result[i] ?? 0;
    while (step < 4) {
...
const module: AlgorithmModule = {
    ...
    run,
    pseudocode: [
        "start with players lined up for the bracket",
        "play bracket matches to crown each round winner",
        "append each tournament winner to the output",
        "replace winners and replay affected matches",
        "scan winners into final order",
        "done: output lists players in sorted order",
    ],
};
