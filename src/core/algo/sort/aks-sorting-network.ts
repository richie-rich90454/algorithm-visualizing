...
function* run(input: unknown): Generator<VisualFrame, void, unknown> {
    const fallback: number[] = [8, 4, 6, 2, 7, 3, 5, 1];
    ...
    yield {
        ...
        edges: [],
        description: "Network inputs before layer one.",
        codeLineNumber: 0,
        layout: "array",
    ...
    let budget = 9;
    let depth = 0;
    for (let size = arr.length; size > 1; size = Math.ceil(size / 2)) {
        depth += 1;
        if (budget > 0) {
            budget -= 1;
            yield {
                ...
                edges: [],
                description: `AKS layer ${depth}: epsilon-halvers pair blocks.`,
                codeLineNumber: 1,
                layout: "array",
                meta: { comparisons, swaps },
            };
        ...
        for (let i = 0; i + size < arr.length; i += 1) {
            const a = arr[i] ?? 0;
            const b = arr[i + size] ?? 0;
            comparisons += 1;
            if (a > b) {
                arr[i] = b;
                arr[i + size] = a;
        ...
        if (budget > 0) {
            ...
            yield {
                ...
                edges: [],
                description: `Halver pass with sep ${size} applied.`,
                codeLineNumber: 2,
                layout: "array",
                meta: { comparisons, swaps },
            };
