 ...
 *
 * `get(u, v)` returns the residual capacity of the u→v edge (which starts at
...
export class ResidualGraph {
    ...
    set(u: string, v: string, capacity: number): void {
        this.caps.set(`${u}→${v}`, capacity);
    }

    get(u: string, v: string): number {
        return this.caps.get(`${u}→${v}`) ?? 0;
    }

    add(u: string, v: string, amount: number): void {
        this.caps.set(`${u}→${v}`, this.get(u, v) + amount);
    }
