 ...
 * directed capacity network. It repeatedly finds *any* sâ†’t path in the
...
function* run(input: unknown): Generator<VisualFrame, void, unknown> {
    ...
    // Residual capacity per directed edge, keyed "uâ†’v". Reverse edges start
    ...
    // ------------------------------------------------------------------
    for (;;) {
        // BFS for any sâ†’t path along edges with residual capacity.
...
const module: AlgorithmModule = {
    ...
    name: "Ford-Fulkerson",
    category: "flow",
    complexity: { time: "O(EÂ·f)", space: "O(V + E)" },
    // Classic small network with two parallel-ish routes Sâ†’T.
    defaultInput: {
        edges: [
            ["S", "A", 3],
            ["S", "B", 2],
